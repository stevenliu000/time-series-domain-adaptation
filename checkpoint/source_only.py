...
model_sub_folder = '/source_only/task_%s_lbl_percentage_%f_scent_%f'%(args.task, args.lbl_percentage, args.scent)
...
labeled_source_x_filename = '/processed_file_not_one_hot_%s_%1.1f_source_known_label_x.npy'%(args.task, args.lbl_percentage)
...
for epoch in range(args.epochs):
    ...
    for batch_id, (source_x, source_y) in tqdm(enumerate(labeled_source_dataloader), total=len(labeled_source_dataloader)):
        ...
        loss = (criterion_classifier(pred, source_y) +
                criterion_centerloss(source_x_embedding, source_y) * args.scent)
        loss.backward()
        optimizerCNet.step()
    ...
    logger.info('Epochs %i: source labeled acc: %f; source unlabeled acc: %f'%(epoch+1, source_acc_label, source_acc_unlabel))
