...
GNet_source = Generator(dim=64*2).to(device)
GNet_target = Generator(dim=64*2).to(device)
...
GNet_source.apply(weights_init)
GNet_target.apply(weights_init)
...
optimizerGNetSource = torch.optim.Adam(GNet_source.parameters(), lr=args.lr_gan)
optimizerGNetTarget = torch.optim.Adam(GNet_target.parameters(), lr=args.lr_gan)
...
for epoch in range(args.epochs):
    ...
    for batch_id, (source_x, source_y) in tqdm(enumerate(labeled_source_dataloader), total=len(labeled_source_dataloader)):
        ...
        pred = CNet(GNet_source(source_x_embedding))
    ...
    if epoch % args.model_save_period == 0:
        ...
        torch.save(GNet_source.state_dict(), args.save_path+model_sub_folder+ '/GNet_source_%i.t7'%(epoch+1))
        torch.save(GNet_target.state_dict(), args.save_path+model_sub_folder+ '/GNet_target_%i.t7'%(epoch+1))
