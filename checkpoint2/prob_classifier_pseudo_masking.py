...
model_sub_folder = '/checkpoint2/prob_classifier_pseudo_masking/task_%s_slp_%f_tlp_%f_sclass_%f_scent_%f_sbinary_loss_%f_pseudo_weight_%f'%(args.task, args.source_lbl_percentage, args.target_lbl_percentage, args.sclass, args.scent, args.sbinary_loss, args.pseudo_weight)
    ...
    if epoch >= args.epoch_begin_prototype:
        ...
        for batch_id in tqdm(range(len(join_dataloader))):
            ...
            for i in range(num_class):
                mask[i*args.num_per_class:(i+1)*args.num_per_class, i*args.num_per_class:(i+1)*args.num_per_class] = 0
