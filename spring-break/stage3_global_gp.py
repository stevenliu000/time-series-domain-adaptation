...
parser.add_argument('--gpweight', type=float, default=10, help='gp weight for WGAN')
...
model_sub_folder = '/stage3_global_gp/task_%s_gpweight_%f_dglobal_%f_critic_%i_sclass_%f'%(args.task, args.gpweight, args.dglobal, args.n_critic, args.sclass)
...
def _gradient_penalty(real_data, generated_data, DNet, num_class, device, args):
        ...
        prob_interpolated = DNet.D(interpolated)
...
#         torch.save(encoder.state_dict(), args.save_path+model_sub_folder+ '/encoder_%i.t7'%(epoch+1))
        torch.save(DNet_global.state_dict(),  args.save_path+model_sub_folder+ '/DNet_%i.t7'%(epoch+1))
