...
for epoch in range(start_epoch, end_epoch, args.intervals*args.model_save_period):
    ...
    mask_source_labeled = torch.zeros(source_y_labeled.size(0), num_class).scatter_(a, source_y_labeled, 1).to(device)
    mask_target_labeled = torch.zeros(target_y_labeled.size(0), num_class).scatter_(a, target_y_labeled, 1).to(device)
    mask_source_unlabeled = torch.zeros(source_y_unlabeled.size(0), num_class).scatter_(a, source_y_unlabeled, 1).to(device)
    mask_target_unlabeled = torch.zeros(target_y_unlabeled.size(0), num_class).scatter_(a, target_y_unlabeled, 1).to(device)
