...
for epoch in range(3, source_acc_label_.shape[0], args.intervals*args.model_save_period):
    ...
    source_y_labeled = torch.empty(0).to(device)
    source_x_unlabeled_embedding = torch.empty(0).to(device)
    source_y_unlabeled = torch.empty(0).to(device)
    target_x_labeled_embedding = torch.empty(0).to(device)
    target_x_labeled = torch.empty(0).to(device)
    target_x_unlabeled_embedding = torch.empty(0).to(device)
    target_y_unlabeled = torch.empty(0).to(device)
    with torch.no_grad():
    ...
    if args.classifier:
        ...
        while i < args.classifier_epoch or (acc_source_labeled_classifier < 98 and acc_target_labeled_classifer < 98):
            ...
            acc_target_labeled_classifier = (pred.argmax(-1) == target_y_labeled).sum().item()
